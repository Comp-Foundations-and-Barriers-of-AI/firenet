"""
This script computes a perturbation meant to simulate worst case effect for the 
FIRENET on ellipses images. 
"""
...
im_id = cgf['DATA']['im_id']
ell2_err = cgf['DATA']['ell2_err']
...
image = Image.open(join(src_data, 'images', f'sample_{im_id}.png')); # im_data 
image = np.asarray(image, npdtype)/255;
print('np.amax(image): ', np.amax(image));

norm_image = np.linalg.norm(samp*np.fft.fft2(image)/N, 'fro');
print('norm_image: ', norm_image);
max_norm = (ell2_err/100)*norm_image;
...
count = read_count(fname='COUNT_ell.txt')
...
with tf.compat.v1.Session() as sess:
    ...
    scipy.io.savemat(join(dest_data_full, f'im_experi_{count:03}_noiseless.mat'), 
            {'image': image, 'image_rec': noiseless})
    
    fname_out_rec  = join(dest_plots_full, f'im_experi_{count:03}_rec_noiseless.png'); 
    fname_out_orig = join(dest_plots_full, f'im_experi_{count:03}_orig_noiseless.png'); 
    ...
    while(i < (max_num_noise_iter+1) and length < max_norm):
        ...
        print(f'{i:3}/{max_num_noise_iter}, Norm: {length:10g} / {max_norm:10g}, time (min): {t:.2f}, |f(Ax) - f(A(x+r))|: {diff_mapping}')

        fileID.write(f'itr: {i}, norm: {length}, target norm: {max_norm}, |f(Ax) - f(A(x+r))|: {diff_mapping}\n')

        # Start old while loop.
        if length > max_norm:
            scipy.io.savemat(join(dest_data_full, f'im_experi_{count:03}.mat'), 
                    {'image': image, 'image_rec': im_rec, 'rr': rr_save, 'im_adjoint': im_adjoint})

            fname_out_rec = join(dest_plots_full, f'im_experi_{count:03}_rec.png')
            fname_out_adjoint = join(dest_plots_full, f'im_experi_{count:03}_adjoint.png')
            fname_out_orig_p_noise = join(dest_plots_full, f'im_experi_{count:03}_orig_p_noise.png')
