import numpy as np;
from .adversarial_tools import pertub_SGA, pertub_SGA_data_consistent, l2_norm_of_tensor
...
""" % ( self.max_itr,
    ...
    def find_data_consisten_perturbation(self, f, dQ, extract_null_space, batch):
        ref_string = self.reference;
        if type(ref_string) is np.ndarray:
            ref_string = 'ndarray';
        if (self.verbose):
        ...
        if self.optimizer == 'SGA':
            ...
            rr, vv, backlog = pertub_SGA_data_consistent(f, dQ1, batch, self.max_itr, 
                                         self.r[-1], 
                                         self.v[-1],
                                         extract_null_space,
