...
def remove_null_space(image, k_mask_idx1, k_mask_idx2):
    """ Remove the null space part of an image.

    Given a sampling operator A ∈ ℂ^{m × N} with m < N satisfying AA^* = I and an image x ∈ ℂ^N, 
    we can write x as 
        x = x_n + x_p,
    where x_n lies in the null space of A and x_p lies in the orthogonal complement of the null space. 
    This function returns the component x_p for the AUTOMAP (subsampled) Fourier operator.

    :param image: (N x N) matrix
    :param k_mask_idx1: Row indices of k-space samples
    :param k_mask_idx2: Column indices of k-space samples
    """
    ...
    #print('image.shape: ', image.shape, 'samp.shape: ', samp.shape)
    Y =  adjoint_of_samples(sample_image(image_cp, k_mask_idx1, k_mask_idx2), k_mask_idx1, k_mask_idx2);
    return Y

def extract_null_space(image, k_mask_idx1, k_mask_idx2):
    """ Extract the null space part of an image.

    Given a sampling operator A ∈ ℂ^{m × N} with m < N satisfying AA^* = I and an image x ∈ ℂ^N, 
    we can write x as 
        x = x_n + x_p,
    where x_n lies in the null space of A and x_p lies in the orthogonal complement of the null space. 
    This function returns the component x_n for the AUTOMAP (subsampled) Fourier operator.
    ...
    Y =  image_cp - remove_null_space(image, k_mask_idx1=k_mask_idx1, k_mask_idx2=k_mask_idx2);
    return Y
