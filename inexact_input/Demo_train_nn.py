"""
This script trains a single LISTA network to solve the square-root LASSO 
problem.

All hyper-parameters are set in the file `config.yml` and is read by this script.

If the hyper-parameter 'use_count' is true, then this script read the number in
the file COUNT.txt and use this as a model number for storage. The number in
the file COUNT.txt is increase by 1. 

The trained model and the used configuration file are stored under `models/model_{xxx}`
"""
...
learning_rate = eval(cgf['TRAIN']['learning_rate'])
