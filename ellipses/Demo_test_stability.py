...
im = np.asarray(Image.open(join(path_im, f'sample_{attack_idx}.png')), dtype=np_dtype);
im /= 255;
...
me = opA.adjoint(opA(images), output_real=True)

radius = 0.05*compute_radius(opA, images)

radius = float(np.squeeze(radius.numpy()));

opt1 = tf.keras.optimizers.SGD(learning_rate=0.002, momentum=0.9, nesterov=True)

ee, rr = find_adv_pert(opA, model1, images, opt1, 
                       lam=0.002,
                       magn_initial_noise = 1e-3,
                       max_nbr_epochs=20,
                       proj_ell2_ball=False,
                       radius=radius)

set_name = 'attack'
for i in range(images.shape[0]):
    save_data_as_im(opA, model1, images+rr, model_nbr, i, set_name, dest);
